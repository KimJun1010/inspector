...
public abstract class BaseLocalInspectionTool extends AbstractBaseJavaLocalInspectionTool {
     ...
     * @return boolean
     */
    protected boolean checkVariableUseFix(@Nullable PsiElement assignElem, @Nullable PsiElement resolvedElem, @NotNull BaseFixElementWalkingVisitor visitor) {
        PsiMethod method = SecExpressionUtils.getParentOfMethod(assignElem);
        if (method != null) {
            method.accept(visitor);
            return visitor.isFix();
        }

        PsiClassInitializer initializer = SecExpressionUtils.getParentOfClassInitializer(assignElem);
        if (initializer != null) {
    ...
    private boolean checkConstructorHasFix(PsiClass aClass, BaseFixElementWalkingVisitor visitor) {
    ...
    private boolean checkStaticInitializersHasFix(PsiClass aClass, BaseFixElementWalkingVisitor visitor) {
        PsiClassInitializer[] initializers = aClass.getInitializers();
        for(PsiClassInitializer initializer : initializers) {
            if (initializer.hasModifierProperty(PsiModifier.STATIC)) {
                initializer.accept(visitor);
