 ...
 */
public class LDAPUnserialize extends BaseLocalInspectionTool {
    ...
    public @NotNull PsiElementVisitor buildVisitor(@NotNull ProblemsHolder holder, boolean isOnTheFly) {
        return new JavaElementVisitor() {
            ...
            public void visitMethodCallExpression(PsiMethodCallExpression expression) {
                if (SecExpressionUtils.hasFullQualifiedName(expression, "javax.naming.directory.SearchControls", "setReturningObjFlag")) {
                    PsiExpressionList expressionList = expression.getArgumentList();
                    PsiExpression[] args = expressionList.getExpressions();
                    if (args.length == 1 &&
