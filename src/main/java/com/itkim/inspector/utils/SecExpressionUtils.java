package com.itkim.inspector.utils;
...
public class SecExpressionUtils {
    ...
    public static boolean isSqliCareExpression(PsiExpression psiExpression) {
        ...
        if (psiExpression instanceof PsiMethodCallExpression) {
            if (SecExpressionUtils.hasFullQualifiedName((PsiMethodCallExpression)psiExpression, "java.lang.String", "join")) {
                ...
                return false;
            } else if (SecExpressionUtils.hasFullQualifiedName((PsiMethodCallExpression)psiExpression, "org.apache.commons.lang.StringUtils", "join") ||
                    SecExpressionUtils.hasFullQualifiedName((PsiMethodCallExpression)psiExpression, "org.apache.commons.lang3.StringUtils", "join")
            ){
                PsiExpression[] args = ((PsiMethodCallExpression)psiExpression).getArgumentList().getExpressions();
    ...
    public static List<PsiExpression> deconPolyadicExpression(PsiPolyadicExpression expression) {
        ...
        if (expression.getOperationTokenType().equals(JavaTokenType.PLUS)) {
            for (PsiExpression operand: expression.getOperands()) {
                ...
                PsiField field = SecExpressionUtils.resolveField(operand);
                if (field != null) {
    ...
    private static List<PsiReference> getReferenceOnMethodScope(PsiReference reference, int maxOffset) {
        ...
        PsiMethod method = SecExpressionUtils.getParentOfMethod(element);
        if (method == null) {
            return refResults;
        }
